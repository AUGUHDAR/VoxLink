...
// 磁盘STUN缓存，APP端用用户目录替代MC gameDirectory
public class StunCache {
    ...
    private static Path getCachePath() {
        ...
        File voxlinkDir = new File(home, ".voxlink-app");
    ...
    public static void save(String natType, String mappedIp, int mappedPort, List<String> stunUrls) {
        try {
            ...
            AppContext.LOGGER.info("[StunCache] 已保存: NAT={}, mapped={}:{}" , natType, mappedIp, mappedPort);
        } catch (Exception e) {
            AppContext.LOGGER.debug("[StunCache] 保存失败: {}", e.getMessage());
        }
    }
