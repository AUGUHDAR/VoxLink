...
public class P2PBridge {
    ...
    // APP端B端本地桥固定25565起递增, 玩家在MC里直连localhost:25565
    private static final int JOINER_DEFAULT_PORT = 25565;
    private static final int JOINER_PORT_MAX_TRIES = 100;
    ...
    // 从startPort起找可用端口, 占用则+1递增, 直接返回已绑定的ServerSocket, 失败返回null
    private static ServerSocket bindAvailablePort(int startPort) {
        for (int i = 0; i < JOINER_PORT_MAX_TRIES; i++) {
            ...
            try {
                ...
                LOGGER.info("端口{}被占用, 试下一个", port);
            }
        }
        LOGGER.error("从{}起连续{}个端口都被占用 :(", startPort, JOINER_PORT_MAX_TRIES);
        return null;
    }
    ...
    private static CompletableFuture<Integer> connectToHostWithRetry(String hostIp, int hostPort, int attempt) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                ...
                ServerSocket js = bindAvailablePort(JOINER_DEFAULT_PORT);
                if (js == null) {
                    LOGGER.error("Joiner: 找不到可用端口起本地桥 :(");
                    running.set(false);
                    return -1;
                }
    ...
    private static CompletableFuture<Integer> connectToHostIpv6WithRetry(String hostIpv6, int hostPort, int attempt) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                ...
                ServerSocket js = bindAvailablePort(JOINER_DEFAULT_PORT);
                if (js == null) {
                    LOGGER.error("Joiner: 找不到可用端口起IPv6本地桥 :(");
                    running.set(false);
                    return -1;
                }
                int jp = js.getLocalPort();
    ...
    public static synchronized int startUdpJoinerBridge(ReliableUdpTransport transport) {
        ...
        try {
            ...
            ServerSocket js = bindAvailablePort(JOINER_DEFAULT_PORT);
            if (js == null) {
                LOGGER.error("UDP joiner bridge: 找不到可用端口 :(");
                running.set(false);
                return -1;
            }
            joinerServer = js;
            joinerPort = js.getLocalPort();
            running.set(true);
