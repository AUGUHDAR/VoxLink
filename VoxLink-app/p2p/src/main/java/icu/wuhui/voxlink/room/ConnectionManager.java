...
public class ConnectionManager {
    ...
    private volatile String currentRelayPeer = null;
    ...
    // EasyTier: 退避序列末尾加64s, 8次失败后长退避
    private static final int[] BACKOFF_DELAYS_MS = {1000, 1000, 2000, 4000, 4000, 8000, 8000, 16000, 64000};
    ...
    private UdpSocketArray getOrCreateUdpArray(int requiredSize, boolean isEasySym, String stunUrl) {
        ...
        if (cachedUdpArray != null) {
            if (cachedUdpArray.isReusable(requiredSize, isEasySym, now)) {
                AppContext.LOGGER.info("[BirthdayPunch] 复用cached socket数组: {}个socket, age={}ms",
        ...
        for (int i = 0; i < requiredSize; i++) {
            ...
            futures.add(CompletableFuture.supplyAsync(() -> {
                ...
                try {
                    ...
                    AppContext.LOGGER.warn("[BirthdayPunch] 创建socket #{} 失败: {}", idx, e.getMessage());
                    return null;
                }
                try {
        ...
        try {
            ...
            AppContext.LOGGER.warn("[BirthdayPunch] socket数组创建部分超时: {}", e.getMessage());
        }
        ...
        AppContext.LOGGER.info("[BirthdayPunch] 创建新socket数组: {}个socket, easySym={}",
    ...
    public void clearActiveHolePunchers() {
        for (String key : new java.util.ArrayList<>(activeHolePunchers.keySet())) {
            ...
            if (p != null) {
                try { p.close(); } catch (Exception e) { AppContext.LOGGER.debug("cleanup puncher close error: {}", e.getMessage()); }
            }
        }
    ...
    public void clearActiveUdpTransports() {
        for (ReliableUdpTransport t : activeUdpTransports.values()) {
            try { t.close(); } catch (Exception e) { AppContext.LOGGER.debug("cleanup udp transport close error: {}", e.getMessage()); }
        }
    ...
    public void addPeerToBlacklist(String peerId) {
        ...
        AppContext.LOGGER.info("[Connection] 对端{}加入打洞黑名单(1小时)", peerId);
    }
    ...
    public void handleJoinRequest(String from, JsonObject data, int retryCount) {
        ...
        AppContext.LOGGER.info("[RoomManager] 收到join_request from {}, state={}", from, state != null && state != RoomManager.PENDING ? "active" : "null/pending");
        if (state == null || state == RoomManager.PENDING || !state.roomInfo.isHost()) return;

        if (hostPunching || activeHolePunchers.containsKey("host")) {
            if (retryCount >= 3) {
                AppContext.LOGGER.warn("[RoomManager] join_request重试次数用完，丢弃 {}", from);
                return;
            }
            if (activeUdpTransports.containsKey(from)) {
                AppContext.LOGGER.info("[RoomManager] 客户端{}已有活跃transport，忽略重复join_request", from);
                return;
            }
            AppContext.LOGGER.info("[RoomManager] 正在处理打洞，排队join_request from {} (重试{}/3)", from, retryCount + 1);
            ...
            scheduler.schedule(() -> {
                ...
                if (st != null && st != RoomManager.PENDING && st.roomInfo.isHost() && !hostPunching && !activeHolePunchers.containsKey("host")) {
                    if (activeUdpTransports.containsKey(from)) {
                        AppContext.LOGGER.info("[RoomManager] 客户端{}已连接，跳过排队重试", from);
                        return;
                    }
                    AppContext.LOGGER.info("[RoomManager] 重试排队的join_request from {}", from);
        ...
        if (needIp || needIpv6) {
            signalingClient.getPublicIp().thenAccept(ipResponse -> {
                    ...
                    if (st != null && st != RoomManager.PENDING && st.roomInfo.isHost()) {
                        ...
                        if ((st.roomInfo.getHostIpv6() == null || st.roomInfo.getHostIpv6().isEmpty()) && StunDetector.verifyIPv6Connectivity()) {
                            ...
                            if (localIpv6 != null) {
                                ...
                                AppContext.LOGGER.info("[handleJoinRequest] API没返回IPv6，用本地IPv6: {}", localIpv6);
                            }
                        }
                        sendHolepunchOffer(st, from);
                    } else {
                        AppContext.LOGGER.warn("[RoomManager] IP查询期间房间状态变了，用原状态发offer");
                ...
                AppContext.LOGGER.warn("[RoomManager] handleJoinRequest中获取公网IP失败: {}", e.getMessage());
                sendHolepunchOffer(state, from);
                return null;
            });
            return;
    ...
    public void sendHolepunchOffer(RoomManager.RoomState state, String from) {
        ...
        if (localIp != null && !localIp.isEmpty()) {
            ...
            AppContext.LOGGER.info("[RoomManager] 附带主机局域网IP: {}", localIp);
        }
        ...
        if (hostPuncher == null || hostPuncher.getSocket() == null || hostPuncher.getSocket().isClosed()) {
            ...
            try {
                ...
                try {
                    ...
                    AppContext.LOGGER.warn("[RoomManager] 创建主机打洞socket失败: {}", e2.getMessage());
                    hostPuncher = null;
                }
            }
        } else {
            hostPuncher.stopPunch();
            AppContext.LOGGER.info("[RoomManager] 复用已有主机打洞socket(localPort={})", hostPuncher.getSocket().getLocalPort());
        }
        ...
        if (stunProbeResult != null && state.roomInfo.getClientId() != null) {
            ...
            if (hostMappedIp != null && hostMappedPort > 0) {
                boolean relayOk = AppContext.getConfig().isRelayEnabled();
        ...
        CompletableFuture.supplyAsync(() -> {
            ...
            if (fHostPuncher != null) {
                try {
                    ...
                    AppContext.LOGGER.info("[RoomManager] 主机NAT: {} — 8并发竞速STUN({}服务器)", fNatType != null ? fNatType : "null", allStun.size());
                    StunProbe.PublicMappedAddress[] top2 = StunProbe.discoverMappedAddressRace(
                            fHostPuncher.getSocket(), allStun, 2);
                    m1 = top2[0];
                    m2 = top2[1];
                } catch (Exception e) {
                    AppContext.LOGGER.warn("[RoomManager] 打洞socket STUN失败: {}", e.getMessage());
                }
                ...
                if (fIsSymmetricOrUnknown && m1 != null && m2 != null) {
                    ...
                    AppContext.LOGGER.info("[RoomManager] 对称NAT, 预创建{}个birthday socket纳入holepunch_offer", birthdayCount);
                    ...
                    try {
                        ...
                        AppContext.LOGGER.warn("[RoomManager] birthday socket创建部分超时: {}", e.getMessage());
                    }
                    for (var f : bFutures) {
                        try {
                    ...
                    AppContext.LOGGER.info("[RoomManager] 预创建{}个birthday socket完成, {}个有效", birthdayCount, birthdayAddrs.size());
                }
            }
            ...
            if (mapped1 != null && mapped2 != null) {
                if (mapped1.port() != mapped2.port()) {
                    ...
                    AppContext.LOGGER.info("[RoomManager] 打洞socket STUN: 对称NAT ({} vs {})", mapped1.port(), mapped2.port());
                } else {
                    if (symOrUnknown && !StunDetector.isNatTypeSymmetric(fNatType)) {
                        AppContext.LOGGER.info("[RoomManager] 打洞socket STUN: 同端口{}，覆盖isSymmetricOrUnknown(原{})", mapped1.port(), fNatType);
                        symOrUnknown = false;
                    }
                }
                mapped = mapped2;
                ...
                if (stunProbeResult != null && !stunProbeResult.serverResults.isEmpty()) {
                    for (StunProbe.StunServerResult sr : stunProbeResult.serverResults) {
                        if (sr.reachable && sr.mappedIp != null && sr.mappedPort > 0) {
                            ...
                            AppContext.LOGGER.info("[RoomManager] MC端口STUN失败, 用NAT探测映射地址兜底: {}:{}", sr.mappedIp, sr.mappedPort);
                            break;
                        }
                    }
                }
                ...
                if (mapped1 != null && mapped2 != null && mapped1.port() != mapped2.port()) {
                    ...
                    if (fHostPuncher != null && fHostPuncher.getSocket() != null) {
                        ...
                        if (samples.size() >= 5) {
                            ...
                            AppContext.LOGGER.info("[RoomManager] P-PRE采样: {}次→序列={}, 可靠delta={} (原2采样delta={})",
                                    samples.size(), samples, reliableDelta, delta);
                            delta = reliableDelta;
                        } else {
                            AppContext.LOGGER.warn("[RoomManager] P-PRE采样不足({}次), 回退2采样delta={}", samples.size(), delta);
                        }
                    }
                    fOfferData.addProperty("hostMappedPortDelta", delta);
                    AppContext.LOGGER.info("[RoomManager] 对称NAT端口偏移: delta={}", delta);
                }
                fState.roomInfo.setHostMappedAddress(mapped.ip(), mapped.port());
                fState.roomInfo.setHostEasySym(hostEasySym);
                AppContext.LOGGER.info("[RoomManager] 打洞socket STUN: ip={}, port={}, symmetric={}, easySym={}",
            ...
            if (birthdayPorts != null && !birthdayPorts.isEmpty()) {
                ...
                AppContext.LOGGER.info("[RoomManager] holepunch_offer附带{}个birthday端口", birthdayPorts.size());
            }
            ...
            AppContext.LOGGER.info("[RoomManager] RTT同步: punchSyncTimeMs={}", syncTime);

            AppContext.LOGGER.info("[RoomManager] 发holepunch_offer给{} (hostIp={}, hostIpv6={}, port={}, mappedIp={}, mappedPort={})",
                    fFrom,
            ...
            signalingClient.sendSignal(fState.roomInfo.getCode(), fState.roomInfo.getToken(), true,
                    ...
                    .thenAccept(response -> {
                        if (!response.success) {
                            AppContext.LOGGER.error("[RoomManager] 发holepunch_offer失败: {} - {}", response.error, response.message);
                        }
                    })
                    .exceptionally(e -> {
                        AppContext.LOGGER.error("[RoomManager] 发holepunch_offer网络错误: {}", e.getMessage());
                        return null;
                    });
        });
    }
    ...
    public void handleHolePunchOffer(String from, JsonObject data) {
        RoomManager.RoomState state = roomManager.currentRoom.get();
        AppContext.LOGGER.info("[RoomManager] 收到holepunch_offer, state={}", state != null && state != RoomManager.PENDING ? "active" : "null/pending");
        if (state == null || state == RoomManager.PENDING || state.roomInfo.isHost()) return;

        if (ConnectionHelper.isConnecting()) {
            AppContext.LOGGER.info("[RoomManager] 已在连接中，忽略holepunch_offer");
            return;
        }

        if (!connectionCycleActive.compareAndSet(false, true)) {
            AppContext.LOGGER.info("[RoomManager] 连接周期已在进行，忽略重复holepunch_offer");
            return;
        }
        ...
        if (P2PBridge.isRunning()) {
            ...
            if (existingPort > 0 && P2PBridge.isTargetMatch(bridgeHostIp, state.roomInfo.getHostPort())) {
                AppContext.LOGGER.info("[RoomManager] 桥已在跑且目标相同，忽略重复holepunch_offer");
        ...
        if (hostMappedIp != null && !hostMappedIp.isEmpty() && hostMappedPort > 0) {
            ...
            AppContext.LOGGER.info("[RoomManager] 更新roomInfo的hostMapped={}:{}", hostMappedIp, hostMappedPort);
        }
        ...
        if (data.has("hostLocalIp") && !data.get("hostLocalIp").isJsonNull()) {
            ...
            AppContext.LOGGER.info("[RoomManager] 主机提供了局域网IP: {}", hostLocalIp);
        }
        ...
        if (punchSyncTime > 0) {
            ...
            AppContext.LOGGER.info("[RoomManager] RTT同步: punchSyncTimeMs={} (距今{}ms)", punchSyncTime, punchSyncTime - System.currentTimeMillis());
        }
        ...
        if (data.has("hostBirthdayPorts") && data.get("hostBirthdayPorts").isJsonArray()) {
            ...
            AppContext.LOGGER.info("[RoomManager] holepunch_offer包含{}个birthday端口", hostBirthdayPorts.size());
        }
        ...
        if (probeFuture != null && stunProbeResult == null) {
            final RoomManager.RoomState fState = state;
            probeFuture.orTimeout(10, java.util.concurrent.TimeUnit.SECONDS)
                .thenAccept(result -> {
                ...
                .exceptionally(e -> {
                    AppContext.LOGGER.warn("[handleHolePunchOffer] STUN探测还没好，先继续");
                    finishHandleHolePunchOffer(fState, from, finalHostIpv6, finalHostIp, finalHostPort,
                            finalHostMappedIp, finalHostMappedPort, finalHostSymmetric, finalHostMappedPortDelta);
                    return null;
                });
    ...
    private void finishHandleHolePunchOffer(RoomManager.RoomState state, String from, String finalHostIpv6, String finalHostIp, int finalHostPort,
        ...
        if (stunProbeResult != null) {
            if (stunProbeResult.natType.isSymmetric()) {
                state.roomInfo.setConnectionMode("voxlink.connection.connecting");
            }
            AppContext.LOGGER.info("[handleHolePunchOffer] 用探测结果: NAT={}, 可达STUN={}",
            ...
            if (clientPublicIp != null && finalHostIp != null && clientPublicIp.equals(finalHostIp)) {
                AppContext.LOGGER.warn("[handleHolePunchOffer] 同公网IP({}): 双方在同一CGNAT后，P2P直连基本没戏", clientPublicIp);
        ...
        if (effectiveMappedIp == null || effectiveMappedPort <= 0) {
            ...
            if (cachedMappedIp != null && cachedMappedPort > 0) {
                ...
                AppContext.LOGGER.info("[handleHolePunchOffer] 用先到的holepunch_mapped: {}:{}", effectiveMappedIp, effectiveMappedPort);
            }
        }

        if (effectiveMappedIp == null || effectiveMappedPort <= 0) {
            connectionCycleActive.set(false);
            AppContext.LOGGER.info("[handleHolePunchOffer] offer没映射地址，等holepunch_mapped...");
            scheduler.schedule(() -> {
                if (connectionCycleActive.compareAndSet(false, true)) {
                    ...
                    if (mappedIp == null || mappedPort <= 0) {
                        AppContext.LOGGER.warn("[handleHolePunchOffer] holepunch_mapped超时(12s)，没有映射地址也开搞");
    ...
    public void handleHolepunchMapped(String from, JsonObject data) {
        ...
        AppContext.LOGGER.info("[RoomManager] 收到主机映射: {}:{} ports={} (sym={}, delta={})", hostMappedIp, hostMappedPort, hostMappedPorts, hostSymmetric, hostMappedPortDelta);
        state.roomInfo.setHostMappedAddress(hostMappedIp, hostMappedPort);
        if (hostSymmetric) {
            state.roomInfo.setHostSymmetric(true);
        }
        ...
        if (data.has("hostLocalIp") && !data.get("hostLocalIp").isJsonNull()) {
            ...
            if (receivedHostLocalIp != null && !receivedHostLocalIp.isEmpty()) {
                ...
                if (existingLocalIp == null || existingLocalIp.isEmpty()) {
                    ...
                    AppContext.LOGGER.info("[handleHolepunchMapped] 收到host回传的局域网IP: {}", receivedHostLocalIp);
                    ...
                    if (connectionCycleActive.get() && state.roomInfo.isSameCgnat() && !connectionWon.get()) {
                        ...
                        int mcPort = state.roomInfo.getHostPort();
                        AppContext.LOGGER.info("[handleHolepunchMapped] CGNAT: 补充尝试hostLocalIp {}:{}", receivedHostLocalIp, connectPort);
                        ...
                        localFallback.tryIpv4Direct(receivedHostLocalIp, connectPort).thenAccept(result -> {
                            if (roomManager.currentRoom.get() == state && result.success && connectionWon.compareAndSet(false, true)) {
                                AppContext.LOGGER.info("[handleHolepunchMapped] CGNAT hostLocalIp直连赢了");
                        ...
                        mcLocalFallback.tryIpv4Direct(receivedHostLocalIp, mcPort).thenAccept(result -> {
                            if (roomManager.currentRoom.get() == state && result.success && connectionWon.compareAndSet(false, true)) {
                                AppContext.LOGGER.info("[handleHolepunchMapped] CGNAT hostLocalIp MC端口赢了");
                                connectViaBridge(state, result);
                            }
                        });
                    }
...
int updatePort = hostMappedPorts.get(0);
            ...
            AppContext.LOGGER.info("[RoomManager] 更新joiner打洞目标到 {}:{}", hostMappedIp, updatePort);
...
int maxExtra = Math.min(hostMappedPorts.size(), 6);
             for (int i = 1; i < maxExtra; i++) {
                ...
                if (!activeHolePunchers.containsKey(key)) {
                    ...
                    try {
                        ...
                        AppContext.LOGGER.info("[RoomManager] 多端口puncher#{}: {}:{}", i, fHostMappedIp, fExtraPort);
                        extraPuncher.punchWithPortPrediction(fHostMappedIp, fExtraPort, 30).thenAccept(socket -> {
                            ...
                            scheduler.submit(() -> {
                                try {
                                    ...
                                    AppContext.LOGGER.error("[RoomManager] 多端口transport失败: {}", e.getMessage());
                                    winPuncher.close();
                                }
                            });
                        }).exceptionally(e -> {
                            AppContext.LOGGER.debug("[RoomManager] 多端口puncher#{}打洞失败: {}", fIdx, e.getMessage());
                            activeHolePunchers.remove(key);
                            try { extraPuncher.close(); } catch (Exception ignored) {}
                            return null;
                        });
                    } catch (Exception e) {
                        AppContext.LOGGER.warn("[RoomManager] 创建多端口puncher#{}失败: {}", fIdx, e.getMessage());
                    }
                }
            }
        } else if (!connectionCycleActive.get() && !ConnectionHelper.isConnecting()) {
            AppContext.LOGGER.info("[RoomManager] 用映射地址启动连接周期");
            if (connectionCycleActive.compareAndSet(false, true)) {
                String hostIp = state.roomInfo.getHostIp();
                String hostIpv6 = state.roomInfo.getHostIpv6();
    ...
    public void handleHostPunchInfo(RoomManager.RoomState state, String from, JsonObject data) {
        ...
        if (data.has("joinerLocalIp") && !data.get("joinerLocalIp").isJsonNull()) {
            ...
            AppContext.LOGGER.info("[HostPunchInfo] joiner局域网IP: {}", joinerLocalIp);
        }
        if (joinerMappedIp != null) {
            ...
            if (myPublicIp != null && myPublicIp.equals(joinerMappedIp)) {
                state.roomInfo.setSameCgnat(true);
                AppContext.LOGGER.warn("[HostPunchInfo] 同公网IP({}): 双方同一CGNAT", joinerMappedIp);
            }
        }
...
if (joinerMappedPortDelta != 0 && joinerMappedPort > 0) {
            ...
            if (predicted > 0 && predicted <= 65535) {
                AppContext.LOGGER.info("[HostPunchInfo] Joiner EasySym预测: STUN端口={} + delta={} → 预测端口={}", joinerMappedPort, joinerMappedPortDelta, predicted);
        ...
        AppContext.LOGGER.info("[HostPunchInfo] called: joinerMapped={}:{}, delta={}, hostPunching={}, bridgeRunning={}, hostSym={}", joinerMappedIp, joinerMappedPort, joinerMappedPortDelta, hostPunching, P2PBridge.isRunning(), StunDetector.isNatTypeSymmetric(state.roomInfo.getNatType()));

        if (joinerMappedIp == null || joinerMappedPort == 0) {
            AppContext.LOGGER.warn("[RoomManager] 无效的punch_info from {}: 没有映射地址", from);
            return;
        }

        if (connectionWon.get()) {
            AppContext.LOGGER.debug("[HostPunchInfo] already connected, ignoring punch_info");
            return;
        }
        ...
        if (hostPunching) {
            if (punchInfoId.equals(lastPunchInfoId)) {
                AppContext.LOGGER.debug("[RoomManager] 已在打同一目标，忽略重复punch_info");
                return;
            }
            AppContext.LOGGER.info("[RoomManager] 已在打洞，目标变了({} -> {})，忽略新punch_info以免CGNAT IP切换导致误重启", lastPunchInfoId, punchInfoId);
            return;
        }
        lastPunchInfoId = punchInfoId;

        AppContext.LOGGER.info("[RoomManager] 收到punch_info from {}: {}:{}", from, joinerMappedIp, joinerMappedPort);
        ...
        if (isHostSym && joinerSym && (isHostHardSym || joinerHardSym)
            ...
            if (upnpResult.success()) {
                AppContext.LOGGER.warn("[HostPunchInfo] 双方对称NAT且至少一方HardSym(hostHard={}, joinerHard={}), UPnP UDP端口{}映射成功, 继续UDP打洞",
                        isHostHardSym, joinerHardSym, upnpPort);
                // UPnP成功, 继续走UDP打洞流程(不return)
            } else {
                AppContext.LOGGER.warn("[HostPunchInfo] 双方对称NAT且至少一方HardSym(hostHard={}, joinerHard={}), UPnP失败, 放弃UDP走TCP兜底",
                        isHostHardSym, joinerHardSym);
                return;
            }
        }
        if (isHostSym && joinerSym) {
            AppContext.LOGGER.info("[HostPunchInfo] 双方都是EasySym(端口可预测)，继续UDP打洞(EasyTier both_easy_sym)");
        }
        ...
        if (portUnreachable && !isHostSym) {
            AppContext.LOGGER.info("[HostPunchInfo] 主机端口状态={}，升级为20 socket birthday attack", portStatus);
        }
        final int HOST_MULTI_COUNT = isHostSym ? 84 : (portUnreachable ? 20 : 3);
        ...
        CompletableFuture.runAsync(() -> {
            ...
            for (int i = 0; i < fHostMultiCount; i++) {
                ...
                stunFutures.add(CompletableFuture.supplyAsync(() -> {
                    ...
                    AppContext.LOGGER.info("[HostPunchInfo] Socket#{} STUN: {} vs {} (localPort={})", idx,
                            a1 != null ? a1.port() : -1, a2 != null ? a2.port() : -1, fp.getSocket().getLocalPort());
                    return new StunProbe.PublicMappedAddress[]{a1, a2};
                }));
                // 顺序创建，每个socket间隔50ms避免Windows资源耗尽
                if (i < fHostMultiCount - 1) {
                    try { Thread.sleep(50); } catch (InterruptedException ignored) { Thread.currentThread().interrupt(); break; }
                }
            }
            AppContext.LOGGER.info("[HostPunchInfo] 成功创建{}/{}个socket", createdCount, fHostMultiCount);

            // 等待所有STUN完成
            CompletableFuture.allOf(stunFutures.toArray(new CompletableFuture[0])).join();
            ...
            if (mappedAddrs.isEmpty()) {
                AppContext.LOGGER.error("[HostPunchInfo] 所有STUN查询失败");
                return;
            }
            if (connectionWon.get()) {
                AppContext.LOGGER.info("[HostPunchInfo] 连接已建立，丢弃迟到的84-socket打洞任务");
            ...
            if (fRequestHostLocalIp || fState.roomInfo.isSameCgnat()) {
                ...
                if (myLocalIp != null && !myLocalIp.isEmpty()) {
                    ...
                    AppContext.LOGGER.info("[HostPunchInfo] CGNAT场景附带hostLocalIp: {}", myLocalIp);
                }
            }
            ...
            AppContext.LOGGER.info("[HostPunchInfo] holepunch_mapped: {} ports={} (symmetric={})", mappedIp, portsArray, hostPunchSocketSymmetric);
            signalingClient.sendSignal(fState.roomInfo.getCode(), fState.roomInfo.getToken(),
                    true, "holepunch_mapped", symData, fFrom)
                    .exceptionally(e -> { AppContext.LOGGER.debug("holepunch_mapped发送失败: {}", e.getMessage()); return null; });
            ...
            for (UdpHolePuncher p : hostPunchers) {
                p.setOnPeerPunchReceived(addr -> {
                    ...
                    signalingClient.sendSignal(code, token, true, "peer_port", portData, fFrom)
                            .exceptionally(e -> { AppContext.LOGGER.debug("peer_port signal failed: {}", e.getMessage()); return null; });
                });
            }

            ScheduledFuture<?> punchTimeout = scheduler.schedule(() -> {
                if (hostPunching) {
                    AppContext.LOGGER.warn("[RoomManager] 主机打洞超时: {}", clientId);
                    hostPunching = false;
                    lastPunchInfoId = "";
            ...
            AppContext.LOGGER.info("[HostPunchInfo] {}个socket并行打洞到 {}:{} range=±{}", hostPunchers.size(), fJoinerMappedIp, fJoinerMappedPort, hostPortRange);
            ...
            for (int i = 0; i < hostPunchers.size(); i++) {
                ...
                mp.punchWithPortPrediction(fJoinerMappedIp, fJoinerMappedPort, hostPortRange).thenAccept(socket -> {
                    ...
                    AppContext.LOGGER.info("[HostPunchInfo] Socket#{} 打洞成功！", idx);
                    ...
                    scheduler.submit(() -> {
                        try {
                            ...
                            AppContext.LOGGER.error("[HostPunchInfo] transport创建失败: {}", e.getMessage());
                            winPuncher.close();
                        }
                    });
                }).exceptionally(e -> {
                    AppContext.LOGGER.debug("[HostPunchInfo] Socket#{} 打洞失败: {}", idx, e.getMessage());
                    return null;
                });
            }
        }, punchExecutor);
    ...
    public void handleJoinerPunchInfo(RoomManager.RoomState state, String from, JsonObject data) {
        ...
        AppContext.LOGGER.info("[RoomManager] 收到punch_info里的主机映射地址: {}:{}", hostMappedIp, hostMappedPort);
        state.roomInfo.setHostMappedAddress(hostMappedIp, hostMappedPort);

        UdpHolePuncher joinerPuncher = activeHolePunchers.get("joiner");
        if (joinerPuncher != null && connectionCycleActive.get()) {
            AppContext.LOGGER.info("[RoomManager] 更新joiner打洞目标到 {}:{}", hostMappedIp, hostMappedPort);
    ...
    public void handlePeerPort(String from, JsonObject data) {
        ...
        AppContext.LOGGER.info("[RoomManager] 收到peer_port信号: 更新目标到 {}:{}", peerIp, peerPort);
    ...
    public void handleReverseHolepunchOffer(String from, JsonObject data) {
        ...
        if (syncTime > 0) {
            ...
            if (delay > 0 && delay < 15000) {
                AppContext.LOGGER.info("[ReversePunch] RTT同步等待: {}ms后启动主机打洞", delay);
                scheduler.schedule(() -> {
    ...
    private void handleReverseHolepunchOfferDelayed(String from, JsonObject data) {
        ...
        if (activeUdpTransports.containsKey(from)) {
            AppContext.LOGGER.info("[ReversePunch] 已有活跃transport给 {}，忽略reverse_holepunch_offer", from);
            return;
        }
...
if (joinerMappedPortDelta != 0 && joinerMappedPort > 0) {
            int predicted = joinerMappedPort + joinerMappedPortDelta;
            if (predicted > 0 && predicted <= 65535) {
                AppContext.LOGGER.info("[ReversePunch] Joiner EasySym预测: {} + {} = {}", joinerMappedPort, joinerMappedPortDelta, predicted);
                joinerMappedPort = predicted;
            }
        }

        if (joinerMappedIp == null || joinerMappedPort == 0) {
            AppContext.LOGGER.warn("[ReversePunch] 无效的reverse_holepunch_offer: 没有映射地址");
            return;
        }

        AppContext.LOGGER.info("[ReversePunch] Host received reverse_holepunch_offer from {}: {}:{} (joinerSym={})", from, joinerMappedIp, joinerMappedPort, joinerSymmetric);

        UdpHolePuncher existingReverse = activeHolePunchers.get("hostRev");
        if (existingReverse != null && existingReverse.isPunching()) {
            AppContext.LOGGER.info("[ReversePunch] already reverse punching, update target to {}:{}", joinerMappedIp, joinerMappedPort);
        ...
        try {
            ...
            AppContext.LOGGER.error("[ReversePunch] create socket failed: {}", e.getMessage());
            return;
        }
        AppContext.LOGGER.info("[ReversePunch] Host reverse punch socket: localPort={}", puncher.getSocket().getLocalPort());
        ...
        punchExecutor.execute(() -> {
            AppContext.LOGGER.info("[ReversePunch] dual STUN on reverse socket...");
            StunProbe.PublicMappedAddress m1 = null, m2 = null;
            try {
                m1 = fPuncher.discoverMappedAddress(java.util.List.of(StunDetector.getAllStunUrls().get(0)));
                AppContext.LOGGER.info("[ReversePunch] Host reverse STUN #1: ip={}, port={} (localPort={})",
                ...
                AppContext.LOGGER.info("[ReversePunch] Host reverse STUN #2: ip={}, port={} (localPort={})",
                        m2 != null ? m2.ip() : "null", m2 != null ? m2.port() : -1, fPuncher.getSocket().getLocalPort());
            } catch (Exception e) {
                AppContext.LOGGER.warn("[ReversePunch] dual STUN失败: {}", e.getMessage());
            }
            ...
            if (hostMapped1 != null && hostMapped2 != null) {
                if (hostMapped1.port() != hostMapped2.port()) {
                    ...
                    AppContext.LOGGER.info("[ReversePunch] Host punch socket STUN: symmetric detected ({} vs {})", hostMapped1.port(), hostMapped2.port());
                }
            ...
            signalingClient.sendSignal(fState.roomInfo.getCode(), fState.roomInfo.getToken(),
                    ...
                    .exceptionally(e -> { AppContext.LOGGER.debug("[ReversePunch] reverse_punch_info发送失败: {}", e.getMessage()); return null; });
            ...
            int hostPortRange = 30;
            if (hostSymmetric && fJoinerSymmetric) {
                hostPortRange = 0;
            } else if (fJoinerSymmetric && allJoinerPorts.size() > 1) {
                hostPortRange = 0;
                AppContext.LOGGER.info("[ReversePunch] 生日攻击模式: 主机打{}个joiner端口: {}", allJoinerPorts.size(), allJoinerPorts);
            ...
            AppContext.LOGGER.info("[ReversePunch] 主机打洞到joiner {}（range=±{}, hostNat={}, joinerSym={}, birthdayPorts={}）",
            ...
            ScheduledFuture<?> punchTimeout = scheduler.schedule(() -> {
                if (activeHolePunchers.get("hostRev") == currentPuncher) {
                    AppContext.LOGGER.warn("[ReversePunch] 主机反向打洞超时: {}", clientId);
            ...
            if (allJoinerPorts.size() > 1) {
                ...
                AppContext.LOGGER.info("[ReversePunch] 生日攻击: {}个端口扩展到{}个端口（范围 {}-{}）",
                        allJoinerPorts.size(), portList.size(),
                        allJoinerPorts.get(0) - 30, allJoinerPorts.get(allJoinerPorts.size() - 1) + 30);
            ...
            punchFuture.thenAccept(socket -> {
                ...
                AppContext.LOGGER.info("[ReversePunch] 主机反向打洞成功，连接到joiner {}:{}", fRevJoinerMappedIp, fRevJoinerMappedPort);
                ...
                scheduler.submit(() -> {
                    try {
                        ...
                        AppContext.LOGGER.error("[ReversePunch] 主机UDP transport创建失败: {}", e.getMessage());
                        hostPuncherRef.close();
                    }
                });
            }).exceptionally(e -> {
                punchTimeout.cancel(false);
                AppContext.LOGGER.warn("[ReversePunch] 主机反向打洞失败 {}: {}", clientId, e.getMessage());
                currentPuncher.cancel();
                currentPuncher.close();
                activeHolePunchers.remove("hostRev");
                return null;
    ...
    public void handleReversePunchInfo(String from, JsonObject data) {
        ...
        if (!connectionCycleActive.get()) {
            AppContext.LOGGER.info("[ReversePunch] 不在连接周期，忽略reverse_punch_info");
            return;
        }
        ...
        if (existingPuncher != null && existingPuncher.isPunching()) {
            ...
            AppContext.LOGGER.info("[ReversePunch] 已在反向打洞中，更新目标到{}:{}", hostMappedIp, updatePort);
            ...
            if (joinerPuncher != null && joinerPuncher.isPunching() && hostMappedIp != null && updatePort > 0) {
                AppContext.LOGGER.info("[ReversePunch] 同步更新主joiner打洞目标到{}:{}", hostMappedIp, updatePort);
                joinerPuncher.updateTarget(hostMappedIp, updatePort);
            }
            return;
        }
        ...
        if (hostMappedIp == null || hostMappedPort <= 0) {
            AppContext.LOGGER.warn("[ReversePunch] reverse_punch_info中没有主机映射地址");
            return;
        }
        ...
        AppContext.LOGGER.info("[ReversePunch] Joiner收到reverse_punch_info: {}:{}（hostSym={}）", hostMappedIp, hostMappedPort, hostSymmetric);
        ...
        if (!birthdayPunchers.isEmpty()) {
            ...
            if (anyPunching) {
                AppContext.LOGGER.info("[BirthdayPunch] 已在打洞中，更新目标到{}:{}，共{}个socket",
            ...
            AppContext.LOGGER.info("[BirthdayPunch] 启动生日攻击{}个socket打洞到 {}:{}",
        ...
        if (puncher == null || puncher.getSocket() == null || puncher.getSocket().isClosed()) {
            AppContext.LOGGER.warn("[ReversePunch] 没有joiner_reverse puncher可用（puncher={}, socket={}, closed={}）",
        ...
        AppContext.LOGGER.info("[ReversePunch] Joiner反向打洞状态: localPort={}, punching={}",
                puncher.getSocket().getLocalPort(), puncher.isPunching());

        puncher.setOnPeerPunchReceived(addr -> {
            AppContext.LOGGER.info("[ReversePunch] Joiner收到对端打洞包 {}:{} — 发peer_port信号", addr.getAddress().getHostAddress(), addr.getPort());
            ...
            signalingClient.sendSignal(code, token, false, "peer_port", portData, "host")
                    .exceptionally(e -> { AppContext.LOGGER.debug("peer_port信号发送失败: {}", e.getMessage()); return null; });
        });
        ...
        if (joinerIsSymmetric) {
            portRange = 30;
            AppContext.LOGGER.info("[ReversePunch] Joiner是symmetric NAT — 用小范围(±30)打开NAT映射");
        } else if (hostSymmetric) {
        ...
        AppContext.LOGGER.info("[ReversePunch] Joiner打洞到主机 {}:{}（range=±{}, joinerSym={}, hostSym={}）",
        ...
        puncher.punchWithPortPrediction(hostMappedIp, hostMappedPort, portRange, true).thenAccept(socket -> {
            ...
            AppContext.LOGGER.info("[ReversePunch] Joiner反向打洞成功 {}:{}", fHostMappedIp, fHostMappedPort);
            ...
            AppContext.LOGGER.info("[ReversePunch] 实际目标地址: {} (STUN映射: {}:{})", finalTargetAddr, fHostMappedIp, fHostMappedPort);
            ...
            scheduler.submit(() -> {
                try {
                    ...
                    AppContext.LOGGER.error("[ReversePunch] Joiner UDP transport创建失败: {}", e.getMessage());
                    try { puncherRef.close(); } catch (Exception ignored) {}
                    showConnectFailedFinal(state);
                }
            });
        }).exceptionally(e -> {
            AppContext.LOGGER.warn("[ReversePunch] Joiner反向打洞失败: {}", e.getMessage());
            finalPuncher.cancel();
            finalPuncher.close();
    ...
    public void handleTcpSimopenRequest(String from, JsonObject data) {
        ...
        if (connectionWon.get()) {
            AppContext.LOGGER.info("[TcpSimOpen] already connected, ignoring tcp_simopen_request");
            return;
        }

        AppContext.LOGGER.info("[TcpSimOpen] 主机收到joiner {}的请求，尝试TCP连接 {}:{}", from, joinerMappedIp, joinerMappedPort);
        ...
        hostSimFallback.tryTcpSimultaneousOpen(joinerMappedIp, joinerMappedPort, hostPort).thenAccept(result -> {
            if (result.success && connectionWon.compareAndSet(false, true)) {
                AppContext.LOGGER.info("[TcpSimOpen] 主机通过TCP SimOpen连上joiner了！");
                RoomManager.RoomState st = roomManager.currentRoom.get();
                if (st != null) {
                    connectViaBridge(st, result);
                }
            } else if (result.success) {
                AppContext.LOGGER.info("[TcpSimOpen] TCP SimOpen成功但连接已被占，忽略");
            } else {
                AppContext.LOGGER.info("[TcpSimOpen] 主机TCP SimOpen失败: {}", result.failureReason);
            }
        });
    }
    ...
    public void handleHolePunchAnswer(String from, JsonObject data) {
        ...
        if (state != null && state != RoomManager.PENDING && state.roomInfo.isHost()) {
            AppContext.LOGGER.info("收到打洞应答来自joiner {}", from);
        }
    }
    ...
    public void runConnectionCycle(RoomManager.RoomState state, String from, String hostIpv6, String hostIp, int hostPort, String hostMappedIp, int hostMappedPort, int cycle) {
        if (connectionWon.get()) {
            AppContext.LOGGER.info("[Connection] 已连接，跳过周期{}", cycle + 1);
            return;
        }
        ...
        if (cycle == 0) {
            ...
            if (joinerSym || hostSym) {
                ...
                AppContext.LOGGER.info("[Connection] 一方对称NAT(joinerSym={}, hostSym={})，全局超时延长到{}s", joinerSym, hostSym, timeoutSec);
            }
        ...
        if (cycle == 0 && syncTime > 0) {
            ...
            if (delay > 0 && delay < 10000) {
                AppContext.LOGGER.info("[Connection] RTT同步等待: {}ms后同时发包", delay);
                scheduler.schedule(() -> {
    ...
    private void scheduleConnectionTimeout(RoomManager.RoomState state, int timeoutSec) {
        ...
        connectionTimeoutFuture = scheduler.schedule(() -> {
            if (connectionWon.get()) return;
            if (connectionCycleActive.get() && roomManager.currentRoom.get() == state) {
                AppContext.LOGGER.warn("[Connection] 全局超时({}s)，中止", finalTimeoutSec);
                showConnectFailedFinal(state);
            }
    ...
    private void extendConnectionTimeoutIfNeeded(RoomManager.RoomState state) {
        ...
        AppContext.LOGGER.info("[Connection] NAT探测完成发现对称NAT，全局超时延长到{}s(剩余{}ms)", SYMMETRIC_CONNECTION_TIMEOUT_SECONDS, remainingMs);
    ...
    public void tryConnectionStep(RoomManager.RoomState state, String from, String hostIpv6, String hostIp, int hostPort, String hostMappedIp, int hostMappedPort, int cycle, int displayCycle, int maxCycles, int step) {
        ...
        if (connectionWon.get()) {
            AppContext.LOGGER.info("[Connection] 已连接，跳过Wave步骤(cycle={}, step={})", cycle + 1, step);
            return;
        }

        switch (step) {
            case 0: {
                AppContext.LOGGER.info("[Connection] Wave 1: LAN+IPv6+UDP并行 (周期{}/{})", displayCycle, maxCycles);
                state.roomInfo.setConnectionMode("voxlink.connection.connecting");
                ...
                if (hostLocalIp != null && !hostLocalIp.isEmpty() && (sameLan || sameCgnat)) {
                    ...
                    AppContext.LOGGER.info("[Connection] Wave 1: 检测到{}(localIp={})，尝试直连", reason, hostLocalIp);
                    ...
                    wave1Futures.add(lanFallback.tryIpv4Direct(hostLocalIp, hostPort).thenAccept(result -> {
                        if (roomManager.currentRoom.get() == state && result.success && connectionWon.compareAndSet(false, true)) {
                            AppContext.LOGGER.info("[Connection] Wave 1: {}直连赢了", reason);
                    ...
                    if (sameCgnat && !sameLan) {
                        int mcPort = state.roomInfo.getHostPort();
                        AppContext.LOGGER.info("[Connection] Wave 1: CGNAT还尝试localIp MC端口 {}:{}", hostLocalIp, mcPort);
                        ...
                        wave1Futures.add(mcFallback.tryIpv4Direct(hostLocalIp, mcPort).thenAccept(result -> {
                            if (roomManager.currentRoom.get() == state && result.success && connectionWon.compareAndSet(false, true)) {
                                AppContext.LOGGER.info("[Connection] Wave 1: CGNAT localIp MC端口赢了");
                                wave1Settled.set(true);
                                connectViaBridge(state, result);
                            }
                        }));
                ...
                if (hostIpv6 != null && !hostIpv6.isEmpty() && StunDetector.verifyIPv6Connectivity()) {
                    AppContext.LOGGER.info("[Connection] Wave 1: 并行尝试IPv6直连");
                    ...
                    wave1Futures.add(ipv6Fallback.tryIpv6Direct(hostIpv6, hostPort).thenAccept(result -> {
                        if (roomManager.currentRoom.get() == state && result.success && connectionWon.compareAndSet(false, true)) {
                            AppContext.LOGGER.info("[Connection] Wave 1: IPv6直连赢了");
                            wave1Settled.set(true);
                            connectViaBridge(state, result);
                        }
                    }));
                } else if (hostIpv6 != null && !hostIpv6.isEmpty()) {
                    AppContext.LOGGER.info("[Connection] Wave 1: 跳过IPv6（本地无IPv6连接）");
                }

                AppContext.LOGGER.info("[Connection] Wave 1: 尝试UDP打洞");
                ...
                if (reversePunchAttempted.compareAndSet(false, true)) {
                    AppContext.LOGGER.info("[Connection] Wave 1: 同时启动反向打洞");
                ...
                if (!wave1Futures.isEmpty()) {
                    CompletableFuture.allOf(wave1Futures.toArray(new CompletableFuture[0])).thenAccept(v -> {
                        if (roomManager.currentRoom.get() != state) return;
                        if (!wave1Settled.get()) {
                            AppContext.LOGGER.info("[Connection] Wave 1 TCP全挂，进Wave 2");
            ...
            case 1: {
                AppContext.LOGGER.info("[Connection] Wave 2: TCP兜底并行 (周期{}/{})", displayCycle, maxCycles);
                ...
                if (hostMappedIp != null && !hostMappedIp.isEmpty() && hostMappedPort > 0) {
                    ...
                    if (myMappedIp != null && myMappedPort > 0 && signalingClient != null) {
                        ...
                        AppContext.LOGGER.info("[Connection] Wave 2: 发tcp_simopen_request给host ({}:{})", myMappedIp, myMappedPort);
                    }
                ...
                if (hostIp != null && !hostIp.isEmpty()) {
                    ...
                    wave2Futures.add(ipv4Fallback.tryIpv4Direct(hostIp, hostPort));
                }
                ...
                if (state.roomInfo.isSameCgnat()) {
                    ...
                    if (hostLocalIp2 != null && !hostLocalIp2.isEmpty()) {
                        AppContext.LOGGER.info("[Connection] Wave 2: CGNAT场景尝试hostLocalIp {}:{}", hostLocalIp2, hostPort);
                        ConnectionFallback localFallback = new ConnectionFallback();
                        wave2Futures.add(localFallback.tryIpv4Direct(hostLocalIp2, hostPort));
                        int mcPort = state.roomInfo.getHostPort();
                        AppContext.LOGGER.info("[Connection] Wave 2: CGNAT场景尝试localIp MC端口 {}:{}", hostLocalIp2, mcPort);
                        ConnectionFallback mcLocalFallback = new ConnectionFallback();
                ...
                if (hostIpv6 != null && !hostIpv6.isEmpty() && StunDetector.verifyIPv6Connectivity()) {
                    AppContext.LOGGER.info("[Connection] Wave 2: 尝试IPv6直连");
                ...
                if (wave2Futures.isEmpty()) {
                    AppContext.LOGGER.info("[Connection] Wave 2: 没TCP兜底可用，进下一周期");
                ...
                for (CompletableFuture<ConnectionFallback.ConnectResult> future : wave2Futures) {
                    future.thenAccept(result -> {
                        if (roomManager.currentRoom.get() == state && result.success && connectionWon.compareAndSet(false, true)) {
                            AppContext.LOGGER.info("[Connection] Wave 2: {}赢了", result.errorCode);
                            connectViaBridge(state, result);
                        }
                    });
                }
                ...
                CompletableFuture.allOf(wave2Futures.toArray(new CompletableFuture[0])).thenAccept(v -> {
                    if (roomManager.currentRoom.get() != state) return;
                    if (!connectionWon.get()) {
                        AppContext.LOGGER.info("[Connection] Wave 2: TCP兜底全挂，进下一周期");
                        advanceToNextCycle(state, from, hostIpv6, hostIp, hostPort, hostMappedIp, hostMappedPort, cycle, maxCycles);
                    }
                });
                return;
    ...
    public void tryUdpPunch(RoomManager.RoomState state, String from, String hostIpv6, String hostIp, int hostPort, String hostMappedIp, int hostMappedPort, int cycle, int displayCycle, int maxCycles, int attempt) {
        if (connectionWon.get()) {
            AppContext.LOGGER.info("[Connection] 已连接，跳过UDP打洞(cycle={}, attempt={})", cycle + 1, attempt);
            return;
        }
        state.roomInfo.setConnectionMode("voxlink.connection.connecting");
        AppContext.LOGGER.info("[Connection] UDP打洞 周期{}/{}, 尝试{}/{}", displayCycle, maxCycles, attempt, UDP_PUNCH_MAX_ATTEMPTS);
        ...
        if (joinerSym && hostSym && (joinerHardSym || hostHardSym)) {
            AppContext.LOGGER.warn("[Connection] 双方对称NAT且至少一方HardSym(joinerHard={}, hostHard={})，跳过UDP打洞(EasyTier策略)",
                    joinerHardSym, hostHardSym);
            tryConnectionStep(state, from, hostIpv6, hostIp, hostPort, hostMappedIp, hostMappedPort, cycle, displayCycle, maxCycles, 1);
            return;
        }
        if (joinerSym && hostSym) {
            AppContext.LOGGER.info("[Connection] 双方都是EasySym(端口可预测)，继续UDP打洞(EasyTier both_easy_sym)");
        }
        ...
        if (puncher != null && puncher.getSocket() != null && !puncher.getSocket().isClosed()) {
            ...
            AppContext.LOGGER.info("[Connection] 复用打洞socket(port={})", puncher.getSocket().getLocalPort());
        } else {
            ...
            try {
                ...
                AppContext.LOGGER.error("[Connection] 创建UDP打洞socket失败: {}", e.getMessage());
                tryConnectionStep(state, from, hostIpv6, hostIp, hostPort, hostMappedIp, hostMappedPort, cycle, displayCycle, maxCycles, 1);
                return;
            }
            activeHolePunchers.put("joiner", puncher);
        ...
        if (myMapped1 != null && myMapped2 != null) {
            if (myMapped1.port() != myMapped2.port()) {
                ...
                AppContext.LOGGER.info("[Connection] Joiner打洞socket STUN: 对称NAT ({} vs {}, delta={})", myMapped1.port(), myMapped2.port(), joinerMappedPortDelta);
            }
        ...
        if (myMappedAddr == null) {
            AppContext.LOGGER.warn("[Connection] 打洞socket STUN失败，试临时socket兜底(尝试{})", attempt);
            ...
            try {
                ...
                AppContext.LOGGER.warn("[Connection] 临时socket STUN也挂了: {}", e.getMessage());
            } finally {
                if (tmp != null && !tmp.isClosed()) { tmp.close(); }
            }
        }
        if (myMappedAddr != null) {
            AppContext.LOGGER.info("[Connection] 我的映射地址: {}:{} (尝试{})", myMappedAddr.ip(), myMappedAddr.port(), attempt);
            // 注册到全局relay候选池
...
boolean joinerSymmetric = (stunProbeResult != null && stunProbeResult.natType.isSymmetric()) || joinerPunchSocketSymmetric;
            ...
            if (joinerMappedPortDelta != 0) {
                ...
                AppContext.LOGGER.info("[Connection] punch_info带端口偏移: delta={}", joinerMappedPortDelta);
            }
            if (joinerPunchSocketSymmetric) {
                AppContext.LOGGER.info("[Connection] 用打洞socket双STUN结果覆盖joinerSymmetric=true");
            }
            ...
            if (joinerLocalIp != null && !joinerLocalIp.isEmpty()) {
                ...
                AppContext.LOGGER.info("[Connection] punch_info附带joiner局域网IP: {}", joinerLocalIp);
            }
            if (state.roomInfo.isSameCgnat() && (state.roomInfo.getHostLocalIp() == null || state.roomInfo.getHostLocalIp().isEmpty())) {
                ...
                AppContext.LOGGER.info("[Connection] CGNAT场景请求host发送局域网IP");
            }
            signalingClient.sendSignal(state.roomInfo.getCode(), state.roomInfo.getToken(),
                    false, "punch_info", punchData, "host");
        } else {
            AppContext.LOGGER.warn("[Connection] STUN绑定失败，没有映射地址(尝试{})", attempt);
            puncher.close();
            if (attempt < UDP_PUNCH_MAX_ATTEMPTS) {
                long delay = UDP_PUNCH_RETRY_DELAY_MS * (1L << Math.min(attempt - 1, 4));
                AppContext.LOGGER.info("[Connection] {}ms后重试UDP打洞(尝试{}/{})", delay, attempt + 1, UDP_PUNCH_MAX_ATTEMPTS);
                scheduler.schedule(() -> {
                    if (roomManager.currentRoom.get() == state && connectionCycleActive.get()) {
...
int hostMappedPortDelta = state.roomInfo.getHostMappedPortDelta();
        if (state.roomInfo.isHostSymmetric() && hostMappedPortDelta != 0 && targetPort > 0) {
            ...
            if (predictedPort > 0 && predictedPort <= 65535) {
                AppContext.LOGGER.info("[Connection] EasySym端口预测: STUN端口={} + delta={} → 预测端口={}", targetPort, hostMappedPortDelta, predictedPort);
        ...
        AppContext.LOGGER.info("[Connection] UDP打洞目标: {}:{} (hostMappedIp={}, hostMappedPort={}, delta={}, hostIp={}, hostPort={}, 尝试{})",
        ...
        if (targetIp == null || targetIp.isEmpty()) {
            AppContext.LOGGER.warn("[Connection] 没有目标IP，UDP打洞没法打");
            puncher.close();
        ...
        if (joinerIsSymmetric && hostConfirmedNonSymmetric) {
            ...
            // symmetric NAT用方向性预测，范围覆盖预测误差
            if (hostMappedPortDelta != 0) {
                // EasySym: 已做delta偏移，±50覆盖误差
                portRange = 50;
            } else if (cycle == 0) {
                portRange = 100;
            } else {
        ...
        AppContext.LOGGER.info("[Connection] 打洞模式: cycle={}, portRange={} (hostSym={}, joinerSym={})", cycle, portRange, state.roomInfo.isHostSymmetric(), joinerIsSymmetric);
        if (portRange > 0) {
            AppContext.LOGGER.info("[Connection] 端口预测(range=+/-{}) (尝试{}, hostSym={}, joinerSym={}, hostNat={})",
        ...
        int socketCount = 0;
        if (joinerIsSymmetric) {
            socketCount = state.roomInfo.isHostSymmetric() ? 84 : 50;
            AppContext.LOGGER.info("[Connection] joiner对称NAT, 创建{}个多socket打洞", socketCount);
        }
        ...
        if (multiSockets.isEmpty()) {
            AppContext.LOGGER.info("[Connection] Cone端复用STUN socket打洞 (port={})", puncher.getSocket().getLocalPort());
            puncher.setOnPeerPunchReceived(addr -> {
                String code = state.roomInfo.getCode();
                String token = state.roomInfo.getToken();
                JsonObject portData = new JsonObject();
            ...
            puncher.punchWithPortPrediction(fTargetIp, fTargetPort, portRange).thenAccept(socket -> {
                ...
                AppContext.LOGGER.info("[Connection] UDP打洞成功 {} (尝试{})", logTarget, attempt);
                finalPuncher.markSocketTransferred();
                killAllConnectionAttempts();
                finalPuncher.stopPunch();
                final DatagramSocket punchSocket = socket;
                ...
                scheduler.submit(() -> {
                    try {
                        ...
                        AppContext.LOGGER.error("[Connection] 创建UDP传输失败: {}", e.getMessage());
                        try { puncherRef.close(); } catch (Exception ignored) {}
                        showConnectFailedFinal(state);
                    }
                });
            }).exceptionally(e -> {
                AppContext.LOGGER.warn("[Connection] UDP打洞失败(周期{}/{}, 尝试{}/{}): {}", displayCycle, maxCycles, attempt, UDP_PUNCH_MAX_ATTEMPTS, e.getMessage());
                finalPuncher.stopPunch();
                ...
                if ("punch stopped".equals(e.getMessage())) {
                    AppContext.LOGGER.info("[Connection] punch被主动停止，不重试");
                    return null;
                }
                if (activeHolePunchers.get("joiner") != finalPuncher) {
                    AppContext.LOGGER.info("[Connection] puncher已被替换，不重试");
                    return null;
                }
                ...
                if (e.getCause() instanceof icu.wuhui.voxlink.network.FirewallBlockedException || e instanceof icu.wuhui.voxlink.network.FirewallBlockedException) {
                    AppContext.LOGGER.warn("[Connection] 防火墙阻止UDP，跳过重试直接进Wave 2 TCP兜底");
                    try { finalPuncher.close(); } catch (Exception ignored) {}
                    activeHolePunchers.remove("joiner");
                    tryConnectionStep(state, from, hostIpv6, hostIp, hostPort, hostMappedIp, hostMappedPort, cycle, displayCycle, maxCycles, 1);
                    return null;
            ...
            AppContext.LOGGER.info("[Connection] EasyTier方式: {}个socket打洞到 {}:{}", multiSockets.size(), fTargetIp, fTargetPort);
            ...
            leadPuncher.punchMultiSocket(fTargetIp, fTargetPort, multiSockets, multiWon).thenAccept(socket -> {
                ...
                String logTarget = fTargetIp != null && fTargetIp.contains(":") ? "[" + fTargetIp + "]:" + fTargetPort : fTargetIp + ":" + fTargetPort;
                AppContext.LOGGER.info("[Connection] 多socket打洞成功 {} (尝试{})", logTarget, attempt);
                ...
                AppContext.LOGGER.warn("[Connection] 多socket打洞失败(周期{}/{}, 尝试{}/{}): {}", displayCycle, maxCycles, attempt, UDP_PUNCH_MAX_ATTEMPTS, e.getMessage());
                ...
                // 防火墙检测: UDP被阻则跳过所有重试，直接进Wave 2 TCP
                if (e.getCause() instanceof icu.wuhui.voxlink.network.FirewallBlockedException || e instanceof icu.wuhui.voxlink.network.FirewallBlockedException) {
                    AppContext.LOGGER.warn("[Connection] 防火墙阻止UDP(多socket)，跳过重试直接进Wave 2 TCP兜底");
                    for (UdpHolePuncher sp : multiSockets) { try { sp.close(); } catch (Exception ignored) {} }
                    for (int si = 0; si < multiSockets.size(); si++) activeHolePunchers.remove("joiner_ms_" + si);
                    activeHolePunchers.remove("joiner");
                    tryConnectionStep(state, from, hostIpv6, hostIp, hostPort, hostMappedIp, hostMappedPort, cycle, displayCycle, maxCycles, 1);
    ...
    public void advanceToNextCycle(RoomManager.RoomState state, String from, String hostIpv6, String hostIp, int hostPort, String hostMappedIp, int hostMappedPort, int cycle, int maxCycles) {
        if (connectionWon.get()) {
            AppContext.LOGGER.info("[Connection] 已连接，不进下一周期");
            return;
        }
        ...
        int delay = BACKOFF_DELAYS_MS[delayIdx];
        AppContext.LOGGER.info("[Connection] 周期{}/{}失败，{}ms后重试(退避)", cycle + 1, maxCycles, delay);
        scheduler.schedule(() -> {
    ...
    public void startReversePunch(RoomManager.RoomState state) {
        AppContext.LOGGER.info("[ReversePunch] 并行启动反向打洞");
        state.roomInfo.setConnectionMode("voxlink.connection.connecting");
        ...
        if (isSymmetric) {
            ...
            AppContext.LOGGER.info("[ReversePunch] 检测到symmetric NAT — 生日攻击{}个socket（easySym={}）", socketCount, isEasySym);
    ...
    public void startBirthdayPunch(RoomManager.RoomState state, int socketCount, boolean isEasySym) {
        ...
        AppContext.LOGGER.info("[BirthdayPunch] 并行STUN {}个socket（目标={}：{}, easySym={}）",
        ...
        CompletableFuture.supplyAsync(() ->
            ...
            if (udpArray == null || udpArray.punchers.isEmpty()) {
                AppContext.LOGGER.error("[BirthdayPunch] 所有STUN查询都失败了");
                showConnectFailedFinal(state);
                return;
            }
            ...
            if (primaryAddr == null) {
                AppContext.LOGGER.error("[BirthdayPunch] 无有效mappedAddr");
                showConnectFailedFinal(state);
                return;
            }

            AppContext.LOGGER.info("[BirthdayPunch] 准备{}个socket，映射端口: {}", birthdayPunchers.size(), mappedPortList);
            ...
            signalingClient.sendSignal(state.roomInfo.getCode(), state.roomInfo.getToken(),
                    ...
                    .thenAccept(response -> {
                        if (!response.success) {
                            AppContext.LOGGER.error("[BirthdayPunch] 发送reverse_holepunch_offer失败: {}", response.error);
                        }
                    })
                    .exceptionally(e -> {
                        AppContext.LOGGER.error("[BirthdayPunch] 发送reverse_holepunch_offer失败: {}", e.getMessage());
                        return null;
                    });
            ...
            // 先发包才能收包
            AppContext.LOGGER.info("[BirthdayPunch] 立刻开始打洞到 {}:{}（不等reverse_punch_info）",
                    fHostMappedIp, fHostMappedPort);

            scheduler.schedule(() -> {
                if (connectionCycleActive.get() && roomManager.currentRoom.get() == state) {
                    AppContext.LOGGER.warn("[BirthdayPunch] 超时（未建立连接）");
    ...
    public void startSimpleReversePunch(RoomManager.RoomState state) {
        ...
        try {
            ...
            AppContext.LOGGER.error("[ReversePunch] 创建打洞socket失败: {}", e.getMessage());
            return;
        }
        ...
        if (myMapped1 != null && myMapped2 != null) {
            if (myMapped1.port() != myMapped2.port()) {
                ...
                joinerMappedPortDelta = myMapped2.port() - myMapped1.port();
                AppContext.LOGGER.info("[ReversePunch] Joiner打洞socket STUN: 检测到symmetric（{} vs {}, delta={}）", myMapped1.port(), myMapped2.port(), joinerMappedPortDelta);
            }
            myMappedAddr = myMapped2;
        } else {
            myMappedAddr = myMapped1 != null ? myMapped1 : myMapped2;
        ...
        if (myMappedAddr == null) {
            AppContext.LOGGER.warn("[ReversePunch] STUN失败，无法反向打洞");
            puncher.close();
            activeHolePunchers.remove("joiner_reverse");
            return;
        }

        AppContext.LOGGER.info("[ReversePunch] Joiner映射地址: {}:{}（symmetric={}）", myMappedAddr.ip(), myMappedAddr.port(), joinerSymmetric);
        ...
        signalingClient.sendSignal(state.roomInfo.getCode(), state.roomInfo.getToken(),
                ...
                .thenAccept(response -> {
                    if (!response.success) {
                        AppContext.LOGGER.error("[ReversePunch] 发送reverse_holepunch_offer失败: {}", response.error);
                    }
                })
                .exceptionally(e -> {
                    AppContext.LOGGER.error("[ReversePunch] 发送reverse_holepunch_offer失败: {}", e.getMessage());
                    return null;
                });
        ...
        if (hostMappedIp == null || hostMappedIp.isEmpty()) {
            AppContext.LOGGER.warn("[ReversePunch] 没有主机地址，无法立即打洞");
            return;
        }
        ...
        AppContext.LOGGER.info("[ReversePunch] 立即打洞到 {}:{}（range=±{}）", fHostMappedIp, fHostMappedPort, portRange);
        ...
        puncher.punchWithPortPrediction(fHostMappedIp, fHostMappedPort, portRange, true).thenAccept(socket -> {
            ...
            AppContext.LOGGER.debug("[ReversePunch] Joiner反向打洞失败(等reverse_punch_info更新目标后重试): {}", e.getMessage());
...
return null;
        ...
        scheduler.schedule(() -> {
            if (connectionCycleActive.get() && roomManager.currentRoom.get() == state && !connectionWon.get()) {
                AppContext.LOGGER.warn("[ReversePunch] 反向打洞超时");
    ...
    public void startBirthdayPunchPhase2(RoomManager.RoomState state, java.util.List<UdpHolePuncher> birthdayPunchers,
        ...
        for (int i = 0; i < birthdayPunchers.size(); i++) {
            ...
            puncher.setOnPeerPunchReceived(addr -> {
                if (won.get()) return;
                AppContext.LOGGER.info("[BirthdayPunch] Socket #{}收到对端打洞到 {}:{}", idx, addr.getAddress().getHostAddress(), addr.getPort());
            });
            ...
            puncher.punchWithPortPrediction(hostMappedIp, hostMappedPort, portRange, true).thenAccept(socket -> {
                ...
                AppContext.LOGGER.info("[BirthdayPunch] Socket #{} 赢了！连接到 {}:{}", idx, hostMappedIp, hostMappedPort);
                ...
                scheduler.submit(() -> {
                    try {
                        ...
                        AppContext.LOGGER.error("[BirthdayPunch] transport创建失败: {}", e.getMessage());
                        try { puncherRef.close(); } catch (Exception ignored) {}
                        showConnectFailedFinal(state);
                    }
                });
            }).exceptionally(e -> {
                AppContext.LOGGER.debug("[BirthdayPunch] Socket #{} 失败: {}", idx, e.getMessage());
                return null;
            });
        }
        ...
        scheduler.schedule(() -> {
            if (!won.get() && connectionCycleActive.get() && roomManager.currentRoom.get() == state) {
                AppContext.LOGGER.warn("[BirthdayPunch] 全部{}个socket超时", birthdayPunchers.size());
                for (int i = 0; i < birthdayPunchers.size(); i++) {
            ...
            P2PBridge.connectToHostIpv6(remoteHost, remotePort)
                    .thenAccept(localPort -> {
                        if (localPort > 0) {
                            ...
                            state.roomInfo.setConnectionMode("voxlink.connection.connected");
                            ConnectionHelper.connectToServer(localPort, state.roomInfo);
                        } else {
                            connectionCycleActive.set(false);
                            ConnectionHelper.resetConnecting();
                            state.roomInfo.setConnectionMode("voxlink.connection.bridge_start_failed", true);
                            signalingClient.sendSignal(state.roomInfo.getCode(), state.roomInfo.getToken(),
    ...
    public void handleConnectViaBridgeFailed(RoomManager.RoomState state) {
        ...
        try {
            ...
            AppContext.LOGGER.warn("调度器已关闭，同步执行leaveRoom");
            if (roomManager.currentRoom.get() == state && state != RoomManager.PENDING) {
                roomManager.leaveRoom();
            }
        }
    ...
    public void startFallbackMonitor(ConnectionFallback fallback, RoomManager.RoomState state) {
        Thread t = new Thread(() -> {
            ...
            while (!fallback.isSettled() && !fallback.isCancelled()) {
                if (loopCount >= 200) {
                AppContext.LOGGER.warn("回退监控超时（约60秒）");
                    break;
                }
                String status = fallback.getStatusText();
                if (status != null) {
            ...
            String finalStatus = fallback.getStatusText();
            if (finalStatus != null) {
    ...
    public void showConnectFailed(RoomManager.RoomState state) {
        if (AppContext.getConfig().isRelayEnabled() && !state.roomInfo.isConnectionAttemptFailed()) {
            ...
            if (hostSym) {
                AppContext.LOGGER.info("[Connection] 房主对称NAT, 尝试P2P中继");
                state.roomInfo.setConnectionMode("voxlink.relay.trying");
                tryRelay(state);
                return;
            }
        }
    ...
    private void tryRelay(RoomManager.RoomState state) {
        // 防止并发重复relay
        if (currentRelayPeer != null) {
            AppContext.LOGGER.info("[Relay] relay已在尝试中 (current={})，跳过重复", currentRelayPeer);
            return;
        }
        if (state.roomInfo.isHost()) {
            ...
            if (candidates.isEmpty()) {
                AppContext.LOGGER.info("[Relay] 无可用的Cone中继节点 (已排除失败={})", failedRelayPeers.size());
            ...
            currentRelayPeer = relay.clientId;
            RoomInfo.PeerInfo symPeer = null;
            for (RoomInfo.PeerInfo p : state.roomInfo.getPeers()) {
                if (!p.clientId.equals(relay.clientId)) {
                    String nt = p.natType;
            ...
            if (symPeer == null || symPeer.mappedIp == null) {
                AppContext.LOGGER.warn("[Relay] 找不到需要中继的对称NAT玩家");
                currentRelayPeer = null;
                showConnectFailedFinal(state);
                return;
            }
            AppContext.LOGGER.info("[Relay] 选Cone中继={} (负载={}), 目标Sym={}", relay.clientId,
                    relayBridge.getRelayCountForPeer(relay.clientId), symPeer.clientId);
            ...
            // relay超时：失败则标记此peer，尝试下一个
            relayFailoverTask = scheduler.schedule(() -> {
                if (!connectionWon.get() && roomManager.currentRoom.get() == state) {
                    AppContext.LOGGER.warn("[Relay] 中继超时 peer={}, 标记失败尝试下一个", currentRelayPeer);
                    failedRelayPeers.add(currentRelayPeer);
                    currentRelayPeer = null;
            ...
            if (currentRelayPeer != null) return;
            currentRelayPeer = "joiner_requesting";
    ...
    public void handleRelayRequest(String from, JsonObject data) {
        ...
        AppContext.LOGGER.info("[Relay] 收到relay_request, 请求者={}", requestingClientId);
        // 房主端日志提示
        AppContext.LOGGER.info("消息: {}", "voxlink.relay.host_notice");
    ...
    public void handleRelayNotify(String from, JsonObject data) {
        ...
        if (data.has("connected") && data.get("connected").getAsBoolean()) {
                AppContext.LOGGER.info("[Relay] 收到relay_notify(connected), 中继已就绪");
            clearRelayTracking();
            connectionWon.set(true);
            state.roomInfo.setConnectionMode("voxlink.relay.connected_via");
            return;
        }
        ...
        AppContext.LOGGER.info("[Relay] 收到relay_notify, 打洞到Cone {}:{}", relayIp, relayPort);
        state.roomInfo.setConnectionMode("voxlink.relay.trying");
        ...
        for (int i = 0; i < RELAY_SOCKET_COUNT; i++) {
            ...
            rp.punch(fRelayIp, fRelayPort)
                ...
                .thenAccept(socket -> {
                    ...
                    AppContext.LOGGER.info("[Relay] Sym→Cone socket#{} 打洞成功", idx);
                    ...
                    scheduler.schedule(() -> {
                        AppContext.LOGGER.info("消息: {}", "voxlink.relay.connected_via");
                    }, 2, TimeUnit.SECONDS);
                })
                .exceptionally(e -> {
                    if (!relayWon.get()) {
        ...
        scheduler.schedule(() -> {
            if (!relayWon.get() && !connectionWon.get()) {
                AppContext.LOGGER.warn("[Relay] Sym→Cone relay打洞超时");
                for (UdpHolePuncher op : relayPunchers) {
    ...
    public void handleRelaySetup(String from, JsonObject data) {
        if (!AppContext.getConfig().isRelayEnabled()) {
            signalingClient.sendSignal(roomManager.currentRoom.get().roomInfo.getCode(),
                    roomManager.currentRoom.get().roomInfo.getToken(), false, "relay_declined", new JsonObject(), from);
            return;
        }
        ...
        for (int i = 0; i < RELAY_SOCKET_COUNT; i++) {
            ...
            cp.punchWithPortPrediction(fTargetIp, fTargetPort, 10)
                ...
                .thenAccept(socket -> {
                    ...
                    AppContext.LOGGER.info("[Relay] Cone→Sym socket#{} 打洞成功", idx);
        ...
        scheduler.schedule(() -> {
            if (!coneWon.get()) {
                AppContext.LOGGER.warn("[Relay] Cone→Sym relay打洞超时");
                for (UdpHolePuncher op : conePunchers) {
                    try { op.cancel(); op.close(); } catch (Exception ignored) {}
                }
    ...
    public void onRelayDisconnected(String peerA, String peerB) {
        AppContext.LOGGER.warn("[Relay] 中继断开通知: {}<->{}", peerA, peerB);
        RoomManager.RoomState state = roomManager.currentRoom.get();
        ...
        scheduler.schedule(() -> {
            if (roomManager.currentRoom.get() == state && !connectionWon.get()) {
                AppContext.LOGGER.info("[Relay] 自动切换备选中继...");
                tryRelay(state);
            }
    ...
    public void showConnectFailedFinal(RoomManager.RoomState state) {
        ...
        failedRelayPeers.clear();
        if (connectionWon.get()) {
            AppContext.LOGGER.info("[Connection] 已连接，忽略showConnectFailedFinal");
            return;
        }
        connectionCycleActive.set(false);
        connectionWon.set(false);
        ...
        ConnectionHelper.resetConnecting();
        state.roomInfo.setConnectionMode("voxlink.connection.all_failed", true);
        ...
        activeUdpTransports.clear();
        AppContext.LOGGER.info("消息: {}{}", "voxlink.chat.error_prefix", "voxlink.connection.all_failed");
        if (state.roomInfo.isSameCgnat()) {
            AppContext.LOGGER.info("消息: {}", "voxlink.chat.same_cgnat_warning");
        }
        try {
    ...
    public void sendDisconnectOnFailure(RoomManager.RoomState state) {
        try {
            ...
            AppContext.LOGGER.debug("连接失败时发送disconnect失败: {}", e.getMessage());
        }
    }
    ...
    public void sendConfirmPackets(DatagramSocket socket, InetSocketAddress addr) {
        try {
            byte[] data = new byte[]{0x56, 0x4C, 0x02};
            ...
            AppContext.LOGGER.warn("[Transport] 确认包发送失败: {}", e.getMessage());
        }
    }
