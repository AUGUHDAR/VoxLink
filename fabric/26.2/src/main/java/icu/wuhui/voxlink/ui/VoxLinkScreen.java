...
public class VoxLinkScreen extends VoxLinkScreenBase {
    ...
    private void rebuildWidgetsForState() {
        ...
        if (currentRoom != null) {
            if (currentRoom.isHost()) {
                addRenderableWidget(Button.builder(
                        ...
                        button -> Minecraft.getInstance().gui.setScreen(new ManageRoomScreen(VoxLinkScreen.this, currentRoom))
                ).bounds(centerX - BTN_W / 2, topStartY, BTN_W, BTN_H).build());
            }
        } else if (isInSingleplayerWorld()) {
            addRenderableWidget(Button.builder(
                    Component.translatable("voxlink.create_room"),
                    button -> Minecraft.getInstance().gui.setScreen(new CreateRoomScreen(this))
            ).bounds(centerX - BTN_W / 2, topStartY, BTN_W, BTN_H).build());
        } else {
            addRenderableWidget(Button.builder(
                    Component.translatable("voxlink.join_by_code"),
                    button -> Minecraft.getInstance().gui.setScreen(new JoinRoomScreen(this))
            ).bounds(centerX - BTN_W / 2, topStartY, BTN_W, BTN_H).build());

            addRenderableWidget(Button.builder(
                    Component.translatable("voxlink.browse_rooms"),
                    button -> Minecraft.getInstance().gui.setScreen(new RoomBrowserScreenBase(this))
        ...
        addRenderableWidget(Button.builder(
                ...
                button -> Minecraft.getInstance().gui.setScreen(new TerracottaConfigScreen(this))
