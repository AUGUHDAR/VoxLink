...
public class UdpHolePuncher {
    ...
    //优化: 8s足够(早期成功检测+防火墙4s判定), 20s纯浪费用户时间
    private static final int PUNCH_TIMEOUT_MS = 8000;
    private static final int FIREWALL_DETECT_CYCLES = 20; // 4s@200ms, 无回包即判防火墙阻断
    public static final int PORT_PREDICTION_MAX_RANGE = 20;
    // 渐进扩展
    private static final int[] PROGRESSIVE_RANGES = {4, 8, 15, 20};
    ...
    public CompletableFuture<DatagramSocket> punchMultiSocket(String remoteIp, int targetPort,
        ...
        }, PUNCH_TIMEOUT_MS + EXTRA_WAIT_MS, TimeUnit.MILLISECONDS);
        timeoutFuture = tf;
    ...
    public CompletableFuture<DatagramSocket> punchMultiPort(String remoteIp, java.util.List<Integer> targetPorts) {
        ...
        }, PUNCH_TIMEOUT_MS + EXTRA_WAIT_LONG_MS, TimeUnit.MILLISECONDS);
        timeoutFuture = tf;
        P2PBridge.registerPendingUdpTimeout(tf);
    ...
    public CompletableFuture<DatagramSocket> punchEasySymDual(
            String remoteIp, int remoteBasePort,
            StunProbe.NatType localNat, StunProbe.NatType remoteNat) {
        LOGGER.info("[UdpHolePuncher] EasySym对打启动: 目标={}:{}, sockets={}, range=±{}, local={}, remote={}",
                remoteIp, remoteBasePort, EASY_SYM_DUAL_SOCKET_COUNT, EASY_SYM_DUAL_PORT_RANGE,
                localNat.key, remoteNat.key);

        java.util.List<UdpHolePuncher> punchers = new java.util.ArrayList<>();
        for (int i = 0; i < EASY_SYM_DUAL_SOCKET_COUNT; i++) {
        ...
        final java.util.List<CompletableFuture<DatagramSocket>> futures = new java.util.ArrayList<>();
        for (UdpHolePuncher p : punchers) {
            futures.add(p.punchWithPortPrediction(remoteIp, remoteBasePort, EASY_SYM_DUAL_PORT_RANGE, true));
        }

        final CompletableFuture<DatagramSocket> result = new CompletableFuture<>();
        final java.util.concurrent.atomic.AtomicInteger remaining =
                new java.util.concurrent.atomic.AtomicInteger(futures.size());

        for (int i = 0; i < futures.size(); i++) {
            final int idx = i;
            futures.get(i).whenComplete((sock, ex) -> {
                if (sock != null) {
                    //CAS守卫: 首个命中即整体成功
                    if (result.complete(sock)) {
