...
public class P2PBridge {
    ...
    private static volatile long firstPacketDeadlineMs = 0;
    private static final long LAZY_P2P_TIMEOUT_MS = 5000;  //5s兜底
    ...
    private static void acceptHostConnections(ServerSocket ss, int minecraftPort) {
        try {
            ss.setSoTimeout(PUNCH_SOCKET_TIMEOUT_MS);
    ...
    private static void acceptJoinerConnectionPreconnected(ServerSocket js, Socket hostSocket) {
        try {
            js.setSoTimeout(CONNECT_TIMEOUT);
        } catch (IOException ignored) {}
        try {
    ...
    private static void acceptJoinerConnections(ServerSocket js) {
        try {
            js.setSoTimeout(PUNCH_SOCKET_TIMEOUT_MS);
        } catch (IOException ignored) {}
    ...
    private static void bridge(BridgePair pair, Socket from, Socket to, String label) {
        ...
        try {
            from.setSoTimeout(KEEPALIVE_SOCKET_TIMEOUT_MS);
        } catch (IOException ignored) {}
    ...
    private static synchronized ExecutorService getOrCreateExecutor() {
        if (bridgeExecutor == null || bridgeExecutor.isShutdown()) {
            bridgeExecutor = new ThreadPoolExecutor(0, MAX_THREADS, KEEPALIVE_TIME_SEC, TimeUnit.SECONDS,
    ...
    public static void armLazyP2pDeadline() {
        firstPacketDeadlineMs = System.currentTimeMillis() + LAZY_P2P_TIMEOUT_MS;
    }
    ...
    private static void acceptUdpJoinerConnections(ServerSocket js, ReliableUdpTransport transport) {
        try { js.setSoTimeout(PUNCH_SOCKET_TIMEOUT_MS); } catch (IOException ignored) {}
