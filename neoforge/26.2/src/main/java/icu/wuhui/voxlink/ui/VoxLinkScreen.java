...
public class VoxLinkScreen extends VoxLinkScreenBase {
   ...
   private void rebuildWidgetsForState() {
      ...
      if (this.lyWebsite != Integer.MIN_VALUE) {
         this.addRenderableWidget(
            Button.builder(Component.translatable("voxlink.website"), button -> Minecraft.getInstance().gui.setScreen(new RelatedLinksScreen(this))).bounds(centerX - 100, this.lyWebsite, HALF_BTN_W, 20).build()
         );
         this.addRenderableWidget(
            Button.builder(Component.translatable("voxlink.feedback"), button -> Minecraft.getInstance().gui.setScreen(new FeedbackScreen(this))).bounds(centerX + L_GAP, this.lyWebsite, HALF_BTN_W, 20).build()
         );
      }
